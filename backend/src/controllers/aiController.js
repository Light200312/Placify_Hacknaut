...
// --- NEW (PrepInsta-style Schema) ---
// This schema matches your new detailed diagram.
...
// --- NEW (Updated Prep Guide Prompt) ---
// This prompt asks for the new, detailed schema.
...
async function callGeminiAPI(userQuery) {
  // FIX: Removed "three" from the URL
...
async function callGeminiVisionAPI(imageData, mimeType) {
    ...
    contents: [
      {
        ...
        parts: [
          { text: userQuery },
          {
            inlineData: {
              mimeType: mimeType,
              data: imageData // The base64 string
            }
          }
        ]
      }
  ...
  const maxRetries = 2; // Try a total of 2 times
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      const response = await fetch(apiUrl, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
        signal: controller.signal // Pass the AbortSignal for timeout
      });
      ...
      if (result.candidates && result.candidates[0]?.content?.parts[0]?.text) {
        return JSON.parse(result.candidates[0].content.parts[0].text); // Success!
      } else {
      ...
      if (error.name === 'AbortError') {
        ...
        // This was the specific network error
        ...
        // Another type of error (like JSON parsing, 4xx status) - don't retry
        throw error;
      }
      // If we are here, it's a retryable error and we'll loop again
    }
  }
}
...
// --- NEW (Updated Prep Guide Function) ---
/**
...
async function callGeminiPrepGuide(company, jobRole) {
  ...
  const payload = {
    ...
    // This is the magic! Enable Google Search.
    tools: [{ "google_search": {} }],
    generationConfig: {
      temperature: 0.2
    }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      if (result.candidates && result.candidates[0]?.content?.parts[0]?.text) {
        // --- FIX: Extract JSON from the text response ---
        ...
        return JSON.parse(jsonString); // Parse the extracted string
      } else {
 ...
 * Analyzes an uploaded resume (image or PDF) and extracts text.
 */
...
/**
 * [POST] /api/analyze
 * Runs analysis from both Gemini and OpenRouter concurrently.
 */
...
/**
 * [POST] /api/prep-guide
 * Uses Google Search to find hiring info and prep links.
 */
export const getPrepGuide = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json(prepGuideData);
  } catch (error) {
